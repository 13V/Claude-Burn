...
async function checkDexScreener() {
    try {
        ...
        if (response.data && response.data.pairs) {
            ...
            const pumpTokens = pairs.filter((p: any) => p.chainId === 'solana' && p.baseToken.address.endsWith('pump'));
            console.log(`Found ${pumpTokens.length} tokens ending in 'pump'.`);

            pumpTokens.sort((a: any, b: any) => b.priceChange.h24 - a.priceChange.h24);

            pumpTokens.slice(0, 20).forEach((p: any) => {
