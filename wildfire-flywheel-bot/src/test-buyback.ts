...
async function testBuyback() {
    ...
    try {
        ...
        try {
            ...
            console.error('Burn failed in test:', (burnError as any).message);
        ...
        try {
            ...
            console.warn('Telegram notification failed (check CHAT_ID):', (botErr as any).message);
        }
        process.exit(0);
    } catch (e) {
        console.error('Manual buyback failed:', e instanceof Error ? e.message : e);
        if ((e as any).response) {
            console.error('Response data:', JSON.stringify((e as any).response.data));
        } else {
