...
import { PUMP_PROGRAM_ID, TOKEN_MINT, PRIORITY_FEE } from './config.js';
import { logActivity, logError } from './logger.js';
export async function getAccruedFees() {
    if (!TOKEN_MINT)
        return 0;
    try {
        const [revenuePda] = PublicKey.findProgramAddressSync([Buffer.from("creator-revenue"), TOKEN_MINT.toBuffer()], PUMP_PROGRAM_ID);
